...
function Main() {
  ...
  async function getCategories() {
    try {
      const {
        status,
        data: { response },
      } = await axios.get(`${json_config[0].url_connect}/product-categories`);
      if (status === 200) {
  ...
  const validateProductData = () => {
    ...
    const sizePrices = _sizePrice.current.value
      .split(",")
    ...
    const sizeQuantities = _sizeQuantity.current.value
      .split(",")
  ...
  const handleAddProduct = async () => {
    ...
    const sizeArray =
      _size.current.value && _size.current.value.trim() !== ""
        ? _size.current.value.split(",").map((size, index) => {
            ...
            const sizePriceInput = _sizePrice.current.value
              .split(",")
              [index]?.trim();
            const sizeQuantityInput = _sizeQuantity.current.value
              .split(",")
              [index]?.trim();

            const price =
              sizePriceInput &&
              !isNaN(sizePriceInput) &&
              parseFloat(sizePriceInput) > 0
                ? parseFloat(sizePriceInput)
                : 0; // Giá mặc định là 0 nếu không hợp lệ

            const quantity =
              sizeQuantityInput &&
              !isNaN(sizeQuantityInput) &&
              parseInt(sizeQuantityInput, 10) > 0
    ...
    try {
      const {
        status,
        data: { response, type },
      } = await axios.post(`${json_config[0].url_connect}/products/add`, {
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      const {
        status,
        data: { response, type },
      } = await axios.post(`${json_config[0].url_connect}/products/update`, {
        id: dataUpdate._id,
        image: _image.current.value,
        name: _name.current.value,
        status: _status.current.value,
    ...
    if (result) {
      try {
        const {
          status,
          data: { response, type },
        } = await axios.post(`${json_config[0].url_connect}/products/delete`, {
          id: productId,
        });
        if (status === 200) {
          window.alert(response);
          ...
          <div className="item-modal-content">
            ...
            <div className="item-flex-row">
              <div className="item-input-group">
                ...
                <input ref={_size} type="text" placeholder="Ví dụ: M,L,XL" />
              </div>
              <div className="item-input-group">
                <span className="item-input-group-text" style={{ width: 100 }}>
                  Price
