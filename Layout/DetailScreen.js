  ...
  const addToCart = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        ToastAndroid.show(response.data.response || "Có lỗi xảy ra!", ToastAndroid.SHORT);
      }
    } catch (error) {
      console.error(error);
      if (error.response) {
        ToastAndroid.show(error.response.data.response || "Có lỗi xảy ra!", ToastAndroid.SHORT);
      } else {
      ...
      <View style={styles.container}>
        ...
        <View style={{ gap: 16, paddingHorizontal: 40 }}>
          <View style={{ width: 180, padding: 8, borderRadius: 10, backgroundColor: '#825640', alignItems: 'center' }}>
            <Text style={{ color: 'white', fontSize: 15, fontWeight: 'bold' }}>{upperCaseFirstItem(item._id.slice(-5))}</Text>
          </View>

          <Text style={{ fontSize: 24, fontWeight: 'bold', color: '#EB4F26' }}>{numberUtils(item.price)} </Text>
