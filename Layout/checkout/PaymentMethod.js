...
const PaymentMethod = ({ navigation }) => {
          ...
          onPress: async () => {
            try {
              ...
              if (storedPaymentMethods) {
                ...
                await AsyncStorage.removeItem(emailUser + "PaymentSaved");
                Alert.alert("Thành công", "Phương thức thanh toán đã được xóa");
                await fetchPaymentMethods(); // Cập nhật lại danh sách sau khi xóa
              }
            } catch (error) {
  ...
  const ItemPayment = ({ item, index }) => {
    const [checkRemember, setCheckRemember] = useState(false);

    useEffect(() => {
      (async function () {
        let paymentSavedIndex = await AsyncStorage.getItem(
          emailUser + "PaymentSaved"
        );
        if (paymentSavedIndex == index) {
    ...
    return (
      <View>
        <TouchableOpacity
          onPress={() => editPaymentMethod(item, index)}
        ...
        >
          ...
          >
            <TouchableOpacity
              ...
              onPress={async () => deletePaymentMethod(index)} // Gọi hàm xóa khi nhấn nút
            >
              <Image
            ...
            <View style={{ padding: 10 }}>
              ...
              <View style={styles.cardFooter}>
                <View>
                  ...
                  <Text style={styles.cardValueName}>
                    {item.cardHolderName}
                  </Text>
                </View>
        ...
        <View style={styles.checkbox}>
          ...
          <Text style={styles.checkboxText}>
            Sử dụng làm phương thức thanh toán
          </Text>
        </View>
      </View>
    );
    ...
    <View style={styles.container}>
      ...
      <FlatList
        ...
        renderItem={({ item, index }) => (
          <ItemPayment item={item} index={index} />
        )}
